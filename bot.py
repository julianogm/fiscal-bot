...
def p(update, context):
    ...
    if processos == False:
        update.message.reply_text(
            "Não foram encontrados processos envolvendo o parlamentar."
        )
        return
...
def main():
    ...
    dp.add_handler(
        MessageHandler(
            Filters.regex(r"^(/dep_[\d@" + updater.bot.username + "]+)$"), dep_nome_link
        )
    )
    dp.add_handler(
        MessageHandler(
            Filters.regex(r"^(/sen_[\d@" + updater.bot.username + "]+)$"), sen_nome_link
        )
    )
